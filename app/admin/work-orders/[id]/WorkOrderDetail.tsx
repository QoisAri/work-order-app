...
import { markAsDone } from './actions'; // Pastikan path ini menunjuk ke file actions.ts yang benar
...
    setIsLoading(true);
    try {
      // Panggil server action. Jika gagal, ia akan melempar error
      await markAsDone(workOrder.id);
      // Jika berhasil, kode akan lanjut ke sini
      setIsModalOpen(false);
      router.refresh();
    } catch (error) {
      // Error yang dilempar oleh server action akan ditangkap di sini
